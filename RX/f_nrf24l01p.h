...
#define SPI_NRF_FUNCTION SPI_Remappable_Read
...
uint8 nrf_test()
{
    ...
    // read actual value
    ___nrf_read_command(NRFCMD_R_REGISTER | NRFREG_RF_CH, 1, &prev);
    
    // calculate and write new value
    new = (~ res) & 0b01111111;
    ___nrf_send_command(NRFCMD_W_REGISTER | NRFREG_RF_CH, 1, &new);
    
    // check if value wrote right
    ___nrf_read_command(NRFCMD_R_REGISTER | NRFREG_RF_CH, 1, &res);
    res = (res == new ? 1 : 0);
    
    // restore old value
    ___nrf_send_command(NRFCMD_W_REGISTER | NRFREG_RF_CH, 1, &prev);
